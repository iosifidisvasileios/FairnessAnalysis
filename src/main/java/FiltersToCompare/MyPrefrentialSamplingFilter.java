...
public class MyPrefrentialSamplingFilter extends Filter  {
    ...
    private String sa_Deprived;
    private int sa_Index;


    public MyPrefrentialSamplingFilter(String protectedValueName, int protectedValueIndex) {
         if (protectedValueName.equals(" Female")) {
            this.sa_Deprived  = "' Female'";
        }else if(protectedValueName.equals(" Minorities")){
            this.sa_Deprived  = "' Minorities'";
        }else {
            this.sa_Deprived  = protectedValueName;
        }
        this.sa_Index = protectedValueIndex;
...
//        System.out.println("DC\t"+dc+"saDep\t"+sa_Deprived);
            try {
                ...
                Logger.getLogger(MyPrefrentialSamplingFilter.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        flushInput();
        m_NewBatch = true;
...
//        }
    ...
    public void weightCalculation(Instances instances)throws Exception{
        ...
        spList=MyMassaging.sorting(spList,sp,2);
        snList=MyMassaging.sorting(snList,sn,2);
        fpList=MyMassaging.sorting(fpList,fp,1);
        fnList=MyMassaging.sorting(fnList,fn,1);
        restList=MyMassaging.sorting(restList,restCount,1);
...
// 	Filter.batchFilterFile(new MyPrefrentialSamplingFilter(), argv);
//      } else {
//	Filter.filterFile(new MyPrefrentialSamplingFilter(), argv);
//      }
//    } catch (Exception ex) {
